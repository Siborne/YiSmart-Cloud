...
| `YiSmartCloud-member-uniapp/` | 家属端 UniApp 源码（Vue3 + Vite；`npm run build:mp-weixin` → `dist/build/mp-weixin`） |
| `mp-weixin/` | 历史微信小程序编译产物（对照用；日常改 `YiSmartCloud-member-uniapp`） |

## 技术栈摘要

- 后端：Java **11**，Spring Boot **2.5.x**，MyBatis-Plus，PageHelper，Spring Security；接口文档 **Knife4j + Swagger 2**（`io.swagger.annotations`）。
- 管理端：Vue **3**、`script setup`、Element Plus、Pinia、Axios；开发代理 **`/dev-api`** → 后端（见 `YiSmartCloud-front/vite.config.js` 中 `baseUrl`，默认 `http://localhost:8080`）。
- 家属端小程序：`YiSmartCloud-member-uniapp`，直连后端根地址（`VITE_API_BASE`），鉴权头为 **`Authorization`** 携带 **Bearer** JWT。
...
4. 家属端小程序：在 `YiSmartCloud-member-uniapp` 执行 `npm install`、`npm run dev:mp-weixin` 或 `npm run build:mp-weixin`，用微信开发者工具打开编译目录（见该目录 `README.md`）。
